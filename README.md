...
composer install          # PHPCS only; the plugin itself has no runtime dependency
```
...
There is nothing to build. `assets/js/editor.js` and `assets/css/rotate-everything.css` ship as written.
...
`make-json` is required: `wp_set_script_translations()` reads a JSON file whose name carries the MD5 of the script's path, and a `.mo` alone will not reach the editor.
